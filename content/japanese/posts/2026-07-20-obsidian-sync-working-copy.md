...
> [!NOTE]
> **すでにPC側でObsidian Gitの設定が完了している場合**
> すでにPC（Mac/Windows）側でObsidian GitによるGitHub自動同期が設定済みの場合は、このステップは不要です。**この項目をスキップ**して [📱 モバイル（iPhone/iPad）側の設定](#-モバイルiphoneipad側の設定) へ進んでください。
